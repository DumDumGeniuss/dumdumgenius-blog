...
class RollingMenu extends React.Component {
    ...
    render() {
        let self = this
        const style = require('./RollingMenu.scss')
        ...
        return (
        	<div className={style.mainArea}>
                ...
                <span className={style.subTitleText}>{subTitle}</span>
                <Link to={this.props.backPath?this.props.backPath:'/'}>
                    <span className={style.squenceButton} style= { {bottom: '110px', left: '20px', display: self.props.backPath?'inline-block':'none'} }>Back</span>
                </Link>
                <span className={style.squenceButton} style= { {bottom: '60px', left: '20px'} } onClick={ self.rollMenu.bind(self, 1, menus.length) }>Prev</span>
                <span className={style.squenceButton} style= { {bottom: '10px', left: '20px'} } onClick={ self.rollMenu.bind(self, 0, menus.length) }>Next</span>
        	</div>
        )
    }
}
